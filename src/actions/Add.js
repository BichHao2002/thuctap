...
function Add(){
  ...
  <Form className='d-grid gap-1'onSubmit={formik.handleSubmit}>
    <Form.Group className='mb-1' controlId = "forName">
      <Form.Control type="text" placeholder="Enter Name" autoComplete='name'  value={formik.values.name} onChange={formik.handleChange}>
      </Form.Control>
    ...
    <Form.Group className='mb-1' controlId = "forCode">
      <Form.Control type="text" placeholder="Enter Code" autoComplete='code' value={formik.values.code} onChange={formik.handleChange}>
      </Form.Control>
    ...
    <Form.Group className='mb-1' controlId = "forDesc">
      <Form.Control type="text" placeholder="Enter Description" autoComplete='description' value={formik.values.description} onChange={formik.handleChange} >
      </Form.Control>

      {formik.errors.description && formik.touched.description && (<p style={{color: "red"}}>{formik.errors.description}</p>)}

    </Form.Group>
    <Button style={{background:"gray", border: "none"}} type='submit'>Submit</Button>
  </Form>
</div>
)
}
